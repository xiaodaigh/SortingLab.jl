...
function sort_spointer!(svec::Vector{String}, lo::Int, hi::Int, ::StringRadixSortAlg, o::O) where O <: Union{ForwardOrdering, ReverseOrdering}
...
# csvec = copy(svec);
# gc()
# @time sort!(csvec, 1, length(csvec), StringRadixSort, Forward);

# csvec = copy(csvec);
# gc()
# @time SortingAlgorithms.sort!(csvec, 1, length(csvec), StringRadixSort, Forward);